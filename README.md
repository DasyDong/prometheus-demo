...
节选自[k8s_paas](https://github.com/dasydong/k8s_PaaS )
...
#### 本机minikube 运行
minikube start --kubernetes-version=1.19.1
...
# 查询kube-metrics是否正常启动，curl
~]# k get pods -o wide
~]# minikube ssh //进入宿主机minikube
                        _             _
            _         _ ( )           ( )
...
~~~
# 22机器，应用：
# 先看一下宿主机有没有9100端口，登录宿主机
~]# minikube  ssh
~]# netstat -luntp|grep 9100  // 然后从宿主机退出
...
# ingress 这里我并没有用到， 使用的是service nodeport 对外暴露访问
...
(base) prome minikube service -n kube-system list
|-------------|-------------------|--------------------|----------------------------|
|  NAMESPACE  |       NAME        |    TARGET PORT     |            URL             |
|-------------|-------------------|--------------------|----------------------------|
| kube-system | blackbox-exporter | blackbox-port/9115 | http://192.168.64.17:32593 |
| kube-system | hello-minikube    |               8080 | http://192.168.64.17:30300 |
| kube-system | kube-dns          | No node port       |
| kube-system | metrics-server    | No node port       |
|-------------|-------------------|--------------------|----------------------------|
(base) prome

访问 http://192.168.64.17:32593/
...
# 准备prometheus的配置文件：
prometheus]# mkdir /data/k8s_yaml/nfs-volume/prometheus
prometheus]# cd /data/k8s-yaml/nfs-volume/prometheus
prometheus]# mkdir {etc,prom-db}
prometheus]# cd etc/
# 此步骤比较重要， 需要copy k8s ca证书给prometheus用
etc]# kubectl config view

- cluster:
    certificate-authority: /Users/dongshu/.minikube/ca.crt
    server: https://192.168.64.17:8443
  name: minikube

- name: minikube
  user:
    client-certificate: /Users/userone/.minikube/profiles/minikube/client.crt
    client-key: /Users/userone/.minikube/profiles/minikube/client.key


etc]# cp /Users/dongshu/.minikube/ca.crt ca.pem
etc]# cp -a /Users/userone/.minikube/profiles/minikube/client.crt client.pem
etc]# cp -a /Users/userone/.minikube/profiles/minikube/client.key client-key.pem

etc]# 因为要启动volume， 那minikube支持mount本地文件目录到宿主机目录
etc]#  minikube mount data/k8s-yaml/prometheus/nfs-volume/: /data/k8s-yaml/prometheus/nfs-volume/
...
etc]# 因为要启动volume， 那minikube支持mount本地文件目录到宿主机目录
etc]#  minikube mount data/k8s-yaml/grafana/nfs-volume/: /data/k8s-yaml/grafana/nfs-volume/
...
(base) sdong:prometheus-demo:% minikube service -n infra list                                                                                                                                   <main ✗>
|-----------|------------|-------------|----------------------------|
| NAMESPACE |    NAME    | TARGET PORT |            URL             |
|-----------|------------|-------------|----------------------------|
| infra     | grafana    |        3000 | http://192.168.64.17:31326 |
